    ...
    return (
      <div className="min-h-screen bg-[#f7f8f8] flex items-center justify-center p-6">
        <div className="max-w-md rounded-3xl border border-gray-200 bg-white p-8 text-center shadow-sm">
          ...
          <Link to={slug ? `/p/${slug}` : '/'} className="mt-5 inline-flex rounded-xl bg-[#0ea5b0] px-4 py-2 text-sm font-medium text-white">
      ...
      <div className="min-h-screen bg-[#f4f8f8] flex items-center justify-center p-6">
        <div className="max-w-lg rounded-[32px] border border-gray-200 bg-white p-8 shadow-sm text-center">
          ...
          <div className="mt-6 flex justify-center gap-3">
            <Link to={`/p/${slug}`} className="rounded-2xl border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700">
            ...
            <button
              ...
              className="rounded-2xl bg-[#0ea5b0] px-4 py-2 text-sm font-medium text-white"
            >
    ...
    <div className="min-h-screen bg-[#f4f8f8] text-gray-900">
      <main className="mx-auto max-w-5xl px-4 py-10 pb-24 sm:px-6 lg:px-8 lg:pb-10">
        <div className="mb-8 flex items-start justify-between gap-4">
          ...
          <Link to={`/p/${slug}`} className="rounded-2xl border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700">
            Voltar à página
          </Link>
        </div>
          ...
          <form id="public-booking-form" onSubmit={handleSubmit} className="space-y-6">
            <section className="rounded-[28px] border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 grid gap-4 md:grid-cols-2">
                <Input label="Nome completo" value={patientName} onChange={(e) => setPatientName(e.target.value)} required />
                <Input label="WhatsApp" value={patientPhone} onChange={(e) => setPatientPhone(e.target.value)} required />
                <div className="md:col-span-2">
                  <Input label="E-mail" type="email" value={patientEmail} onChange={(e) => setPatientEmail(e.target.value)} />
                </div>
              </div>
            </section>
                    ...
                    {slots.map((slot, index) => {
                      ...
                      return (
                        <button
                          ...
                          className={`rounded-2xl border p-4 text-left transition ${selected ? 'border-teal-300 bg-teal-50' : 'border-gray-200 bg-white hover:border-gray-300'}`}
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <p className="text-sm font-semibold text-gray-900">{slot.displayDate} às {slot.displayTime}</p>
                              <p className="mt-1 text-xs text-gray-500">{slot.professionalName}{slot.specialty ? ` • ${slot.specialty}` : ''}</p>
                            </div>
                            <UserCircle size={18} className={selected ? 'text-[#0ea5b0]' : 'text-gray-300'} />
                          </div>
                        </button>
                      )
                    })}
            ...
            <section className="rounded-[28px] border border-gray-200 bg-white p-6 shadow-sm">
              <TextArea label="Observações" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Opcional. Ex: primeira consulta, retorno, preferência de horário." />
            </section>
          </form>

          <aside className="space-y-6">
            <section className="rounded-[28px] border border-gray-200 bg-white p-6 shadow-sm sticky top-4">
              ...
              <button
                ...
                className="mt-6 w-full rounded-2xl px-4 py-3 text-sm font-semibold text-white disabled:opacity-40"
